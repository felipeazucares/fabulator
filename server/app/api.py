...
from .database import TreeStorage, UserStorage, ProjectStorage
from .models import (
    CreateProject,
    RetrieveProject,
    UpdateProject,
...
class RoutesHelper:
    ...
    async def account_id_exists(self, account_id):
        ...
        try:
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Error occured retrieving details for {account_id} : {e}",
            )
    ...
    async def save_document_exists(self, document_id):
        ...
        try:
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Error occured retrieving count of save documents for document save_id: {self.document_id}: {e}",
            )
    ...
    async def user_document_exists(self, user_id):
        ...
        try:
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Error occured retrieving count of user documents user_id: {self.user_id}: {e}",
            )
...
async def get_tree_root(
    account_id: UserDetails = Security(
        get_current_active_user_account, scopes=["tree:reader"]
    )
) -> dict:
    ...
    if await routes_helper.account_id_exists(account_id=account_id):
        ...
        if tree.contains(id):
            ...
            try:
                ...
                raise HTTPException(
                    status_code=500,
                    detail=f"Error occured building the subtree from the request dict object. {e}",
                )
            try:
                if DEBUG:
...
async def create_node(
    ...
    if request["parent"]:
        ...
        if tree.contains(request["parent"]):
            try:
                ...
                raise HTTPException(
                    status_code=500,
                    detail=f"request['name']:{request['name']}, data:{node_payload}, request['parent']:{request['parent']}:{e} ",
                )
        else:
            raise HTTPException(
                status_code=422,
        ...
        if tree.root is None:
            try:
                ...
                raise HTTPException(
                    status_code=500,
                    detail=f"request['name']:{request['name']}, data:{node_payload}:{e}",
                )
        else:
    ...
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail=f"Error occured saving the working tree to the database: {e}",
        )
    if DEBUG:
        console_display.show_debug_message(message_to_show=f"mongo save: {save_result}")
...
async def update_node(
    ...
    global tree
    if DEBUG:
        console_display.show_debug_message(f"update_node({id}) called")
    routes_helper = RoutesHelper()
    if await routes_helper.account_id_exists(account_id=account_id):
        # test if node exists
        if tree.contains(id):
...
async def delete_node(
    ...
    # todo: probably want to stash the children somewhere first in a sub tree
    # for later use
    DEBUG = bool(os.getenv("DEBUG", "False") == "True")
    routes_helper = RoutesHelper()
    global tree
    if await routes_helper.account_id_exists(account_id=account_id):
        if DEBUG:
            console_display.show_debug_message(f"delete_node({id}) called")
        if tree.contains(id):
            try:
...
async def get_latest_save(
    ...
    DEBUG = bool(os.getenv("DEBUG", "False") == "True")
    if DEBUG:
        console_display.show_debug_message(message_to_show=f"get_latest_save()called")
    global tree
    ...
    if await routes_helper.account_id_exists(account_id=account_id):
        try:
            ...
            raise HTTPException(
                status_code=500,
                detail=f"Error occured loading latest save into working tree: {e}",
            )
    else:
        raise HTTPException(
            status_code=404,
            detail=f"Unable to locate saves for account_id:{account_id}",
        )
    ...
    if await routes_helper.account_id_exists(account_id=account_id):
        if await routes_helper.save_document_exists(document_id=save_id):
            try:
                ...
                raise HTTPException(
                    status_code=500,
                    detail=f"Error occured loading specified save into working tree. save_id: {save_id}: {e}",
                )
        else:
            raise HTTPException(
                status_code=404,
...
async def delete_saves(
    ...
    DEBUG = bool(os.getenv("DEBUG", "False") == "True")
    if DEBUG:
        console_display.show_debug_message(f"delete_saves({account_id},{id}) called")
    global tree
    try:
        db_storage = TreeStorage(collection_name="tree_collection")
...
async def save_user(request: UserDetails = Body(...)) -> dict:
    ...
    if DEBUG:
        console_display.show_debug_message(f"save_user({request}) called")
    try:
    ...
    if hasattr(save_result, "error"):
        raise HTTPException(status_code=422, detail=save_result.message)
    else:
...
async def get_user(
    account_id: UserDetails = Security(
        get_current_active_user_account, scopes=["user:reader"]
    )
) -> dict:
    """get a user's details from users collection"""
    DEBUG = bool(os.getenv("DEBUG", "False") == "True")
    if DEBUG:
        console_display.show_debug_message(f"get_user({account_id}) called")
    routes_helper = RoutesHelper()
    # use the accounts_id checker here replace id references
    if await routes_helper.account_id_exists(account_id=account_id):
        try:
            db_storage = UserStorage(collection_name="user_collection")
            get_result = await db_storage.get_user_details_by_account_id(
                account_id=account_id
            )
        except Exception as e:
            console_display.show_exception_message(
...
async def update_user(
    ...
    DEBUG = bool(os.getenv("DEBUG", "False") == "True")
    if DEBUG:
        console_display.show_debug_message(f"update_user({request}) called")
    routes_helper = RoutesHelper()
    # use the accounts_id checker here replace id references
    if await routes_helper.account_id_exists(account_id=account_id):
        try:
            db_storage = UserStorage(collection_name="user_collection")
            update_result = await db_storage.update_user_details(
...
async def update_password(
    ...
    DEBUG = bool(os.getenv("DEBUG", "False") == "True")
    if DEBUG:
        console_display.show_debug_message(f"update_password({request}) called")
...
async def update_type(
    ...
    DEBUG = bool(os.getenv("DEBUG", "False") == "True")
    if DEBUG:
        console_display.show_debug_message(f"update_type({request}) called")

    if account_id is not None:
        try:
            db_storage = UserStorage(collection_name="user_collection")
            update_result = await db_storage.update_user_type(
                account_id=account_id, user=request
            )
        except Exception as e:
            console_display.show_exception_message(
...
async def update_current_project(
    ...
    try:
        db_storage = UserStorage(collection_name="user_collection")
        update_result = await db_storage.update_user_project(
            account_id=account_id, current_project_id=current_project_id
        )
    except Exception as e:
        console_display.show_exception_message(
            message_to_show=f"Error occured updating user details:{account_id}"
...
async def delete_user(
    ...
    DEBUG = bool(os.getenv("DEBUG", "False") == "True")
    if DEBUG:
        console_display.show_debug_message(f"delete_user({account_id}) called")
    routes_helper = RoutesHelper()
    if await routes_helper.account_id_exists(account_id=account_id):
        try:
            db_storage = UserStorage(collection_name="user_collection")
            delete_result = await db_storage.delete_user_details_by_account_id(
                account_id=account_id
            )
        except Exception as e:
            console_display.show_exception_message(
...
async def get_project(
    project_id: str,
    account_id: UserDetails = Security(
        get_current_active_user_account, scopes=["project:reader"]
    ),
):
    ...
    DEBUG = bool(os.getenv("DEBUG", "False") == "True")
    if DEBUG:
        console_display.show_debug_message(f"get_project({project_id}) called")
    try:
    ...
    if hasattr(project_details, "error"):
        raise HTTPException(
            status_code=404,
            detail=f"Unable to retrieve project: {project_details.message}",
        )
    else:
...
async def create_project(
    account_id: UserDetails = Security(
        get_current_active_user_account, scopes=["project:writer"]
    ),
    request: CreateProject = Body(...),
):
    """create a project"""
    DEBUG = bool(os.getenv("DEBUG", "False") == "True")
    if DEBUG:
        console_display.show_debug_message(f"create_project({request}) called")
    try:
        db_storage = ProjectStorage(collection_name="project_collection")
        # populate the project model with all the user details
        project_to_create = RetrieveProject(
            project_id=pwd_context.hash(request.name),
            name=request.name,
            description=request.description,
...
async def update_project(
    ...
    request: UpdateProject = Body(...),
):
    """update a project"""
    DEBUG = bool(os.getenv("DEBUG", "False") == "True")
    if DEBUG:
        console_display.show_debug_message(f"update_project({request}) called")
    try:
        db_storage = ProjectStorage(collection_name="project_collection")
        ...
        save_result = await db_storage.update_project_details(
            account_id=account_id,
